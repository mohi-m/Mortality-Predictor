...
mortality_dataset <- read.csv("datasets/Weekly_Provisional_Counts_of_Deaths_by_State_and_Select_Causes__2020-2023_20250216.csv")
...
ggplot() +
  geom_line(data = merged_dataset, aes(x = `Week Ending Date`, y = `Average Wind Speed (m/s)`)) +
  geom_line(data = merged_dataset, aes(x = `Week Ending Date`, y = `Non Covid Deaths`/max(`Non Covid Deaths`)*max(`Average Wind Speed (m/s)`)), color = "red") +
